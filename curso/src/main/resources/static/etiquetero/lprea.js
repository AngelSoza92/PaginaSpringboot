...
searchPal.addEventListener('submit', (e) => {
  ...
  fetch('http://10.107.226.241/apis/etiq/plr', {
    method: 'POST',
    headers: {
    ...
    .then(data => {
      ...
      if (statusResponse === 'OK') {
        const etiquetas = data.lprs;
        etiquetas.forEach(function (lpr) {
          var canvas = crearbarra(lpr);
...
function crearbarra(texto) {
  ...
  ctx.drawImage(tempCanvas, bxx, 54, barcodeWidth, barcodeHeight);
  ctx.font = 'bold 80px Arial';
  ctx.fillStyle = 'black';
  ctx.fillText(texto, 164, 336);
  return canvas;
}
