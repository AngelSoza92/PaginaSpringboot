...
searchPal.addEventListener('submit', (e) => {
  ...
  fetch('http://10.107.226.241/apis/etiq/tnp', {
    method: 'POST',
    headers: {
    ...
    .then(data => {
      ...
      if (statusResponse === 'OK') {
        ...
        carros.forEach(function (tnp) {
          var canvas = crearbarra(tnp);
...
function crearbarra(texto) {
  ...
  var barcodeWidth = 325;
  var barcodeHeight = 250;
  ...
  ctx.drawImage(tempCanvas, 75, 85, barcodeWidth, barcodeHeight);
  ctx.font = 'bold 70px Arial';
  ctx.fillStyle = 'black';
  ctx.fillText(texto, 430, 235);
  ctx.font = 'bold 30px Arial';
  ctx.fillText('CLICK AND COLLECT B100', 202, 70);
  return canvas;
}
