...
searchPal.addEventListener('submit', (e) => {
  ...
  fetch('http://10.107.226.241/apis/etiq/tot', {
    method: 'POST',
    headers: {
    ...
    .then(data => {
      ...
      if (statusResponse === 'OK') {
        const etiquetas = data.totes;
        etiquetas.forEach(function (tote) {
          var canvas = crearbarra(tote);
...
function crearbarra(texto) {
  ...
  var barcodeWidth = 500;
  var barcodeHeight = 640;
  ...
  ctx.drawImage(tempCanvas, 50, 80, barcodeWidth, barcodeHeight);
  ctx.font = 'bold 180px Arial';
  ctx.fillStyle = 'black';
  ctx.save();
  // Realiza la rotación de 90 grados
  ctx.rotate(-Math.PI / 2); // Gira en sentido contrario a las agujas del reloj
  // Dibuja el texto invertido verticalmente
  ctx.fillText(texto, -713, 713);
  ctx.font = '35px Courier';
  ctx.fillText('Paris Internet Bodega 100', -662, 760);
...
function printz(filtroz) {
  ...
  if (elementosLi.length > 0) {
    for (var i = 0; i < elementosLi.length; i++) {
      ...
      var zpl = `^XA^LH0,0^FWN^PON^PMN^LRN^FO10,10^GFA,${res.length},${res.length},${res.rowlen},${res.z64}^PQ2^XZ`;
      if (filtroz.length > 0) {
